...
After upgrading the SDK, you need to update the code in your app according to your business scenarios. For details, see [Migrate from v<Vg k = "VSDK_PREVIOUS_RELEASE"/> to v<Vg k = "VSDK_RELEASE"/>](./develop/migration-guide).
...
The SDK supports a new version of AI noise reduction (in comparison to the basic AI noise reduction in v<Vg k = "VSDK_PREVIOUS_RELEASE"/>). The new AI noise reduction has better vocal fidelity, cleaner noise suppression, and adds a dereverberation option. To enable this feature, contact [sales-us@agora.io](mailto:sales-us@agora.io).
